...
@Component({
    ...
    template: `
        <div class="notifications shadow">
            <div *ngIf="_warning" class="entry warning border-active" (keyup.esc)="clearWarning()" tabindex="-1" autofocus>
                <i aria-hidden="true" class="sme-icon sme-icon-clear exit" (click)="clearWarning()" title="Dismiss"></i>
            ...
            <div [hidden]="!_active">
                <div *ngFor="let notification of _notifications; let i = index;" [hidden]="notification._hidden" class="entry background-normal border-active">
                    <i aria-hidden="true" class="sme-icon sme-icon-clear sme-lg exit" (click)="dismiss(i)" title="Dismiss"></i>

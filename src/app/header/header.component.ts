...
@Component({
    ...
    template: `
        <div *ngIf="!isWAC" class="nav background-active sme-focus-zone">
                <button aria-hidden="true" class="sme-icon sme-icon-globalNavButton nav-item nav-options hover-primary2" [attr.title]="this.options.active ? 'Hide Sidebar' : 'Show Sidebar'" (click)="this.options.toggle()" [class.background-primary2]="this.options.active"></button>

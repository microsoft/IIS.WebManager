        ...
        <div *ngIf="model" class="grid-item row" [class.background-editing]="_isEditing">
            <div [class]="_isEditing ? 'col-lg-10 col-md-10 col-sm-10 overflow-visible' : ''">
                ...
                <div *ngIf="_isEditing" id="statusCodes" class="col-xs-12">
                    ...
                    <button class="pull-right background-normal" *ngIf="!!(!(statusCodes && statusCodes.list) && model.status_codes.length > 0 || statusCodes.list && statusCodes.list.length> 0)" (click)="statusCodes.add()" ><i aria-hidden="true" class="sme-icon sme-icon-add color-active" ></i><span>Add</span></button>
                    <fieldset>
                        <string-list #statusCodes="stringList" [(model)]="model.status_codes"></string-list>
                        <button class="background-normal" *ngIf="statusCodes.list.length == 0" (click)="statusCodes.add()"><i aria-hidden="true" class="sme-icon sme-icon-add color-active"></i><span>Add</span></button>
                    </fieldset>
                </div>

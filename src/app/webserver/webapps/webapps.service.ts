...
export class WebAppsService {
    ...
    private handleError(e: ApiError, app: WebApp) {
        ...
        if (e.title && e.title.toLowerCase() == 'forbidden' && e.name == 'physical_path') {
            this._notificationService.warn(`IIS has no access to ${app.physical_path}. if the path exists, use "New File System Mapping" button to map the path to IIS file system.`);
        }
    }
}

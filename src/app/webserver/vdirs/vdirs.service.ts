...
export class VdirsService {
    ...
    private handleError(e: ApiError, vdir: Vdir) {
        ...
        if (e.title && e.title.toLowerCase() == 'forbidden' && e.name == 'physical_path') {
            this._notificationService.warn(`IIS has no access to ${vdir.physical_path}. if the path exists, use "New IIS File System Mapping" button to map the path to IIS file system.`);
        }
    }
}
